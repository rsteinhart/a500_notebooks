...
def do_xi(keep_dict,hit):
    ...
    w_flux = ((wperturb.T*wperturb.T)[:,:,1:]*qvavg_grad).mean(axis=(0,1))
    u_flux = ((uperturb.T*wperturb.T)[:,:,1:]*qvavg_grad).mean(axis=(0,1))
    v_flux = ((vperturb.T*wperturb.T)[:,:,1:]*qvavg_grad).mean(axis=(0,1))
